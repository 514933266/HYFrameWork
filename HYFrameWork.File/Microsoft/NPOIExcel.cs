    ...
    public static class NPOIExcel
    {
        ...
        public static IEnumerable<T> Import<T>(string filePath) where T : class, new()
        {
            ...
            using (Stream stream = System.IO.File.OpenRead(filePath))
            {
                ...
                List<DataTable> dts = Import(stream, type, false);
                list = ReflectionHelper.ToList<T>(dts[0]);
            }
            return list;
        }

        /// <summary>
        /// NPOI方式读取Excel流返回表格集合
        /// </summary>
        ...
        public static List<DataTable> Import(Stream stream, FileType type = FileType.xlsx, bool isFirstTitle = false)
        {
        ...
        /// <summary>
        /// NPOI方式读取Excel流返回对象集合
        /// </summary>
        /// <typeparam name="T">读取的Excel流</typeparam>
        /// <param name="postedFile">Excel的类型</param>
        /// <param name="type">第一行是否标题列</param>
        /// <param name="isFirstTitle">表格集合</param>
        /// <returns></returns>
        public static IEnumerable<T> HttpImport<T>(HttpPostedFileBase postedFile, FileType type = FileType.xlsx, bool isFirstTitle = true) where T : class, new()
        {
            List<DataTable> dts = Import(postedFile.InputStream, type, isFirstTitle);
            if (dts.Count > 0)
            {
                return ReflectionHelper.ToList<T>(dts[0]);
            }
            return null;
        }
        ...
        /// Http导出Excel
        /// </summary>
        /// <param name="dts">表格集合</param>
        /// <param name="type">输出类型</param>
        /// <param name="fileName">文件名</param>
        /// <param name="noWriteColumns">不被写入的列集合</param>
        /// <param name="isWriteColumnHeader">是否将列标题写入</param>
        public static void HttpExport(List<DataTable> dts, string fileName, FileType type = FileType.xlsx, int[] noWriteColumns = null, bool isWriteColumnHeader = false)
        {
            var extension = type == FileType.xls ? ".xls" : ".xlsx";
            ...
            HttpContext.Current.Response.ContentType = HttpMIMEType.Excel.ToMIMEString();
            var workbook = Export(dts, type, noWriteColumns, isWriteColumnHeader);
